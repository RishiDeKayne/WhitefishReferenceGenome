#PhaseSynteny_WR2_MOLECOLRES.R
#Updated analysis of wtdbg2 genome assembly for De-Kayne, Zoller and Feulner 2020
...
circos.clear()
#name of file/parameters
tiff("/Users/rishidek/Dropbox/RishiMAC/Genome/PostPhaseScaffoldingFastas/FIG5.tiff",height=8,width=8,units="in",res=300,compression="lzw")
# Customize plot parameters
par(fig=c(0,1,0,1),mar=c(1,1,1,1))
#create gaps vector
gapsnorm <- c(1)
...
Snamevector <- c("WFS40", "WFS38", "WFS22", "WFS32", "WFS28", "WFS31", "WFS35", "WFS06", "WFS39",
                 "WFS18", "WFS03", "WFS02", "WFS24", "WFS37", "WFS34", "WFS16", "WFS36", "WFS33", "WFS14", "WFS23", "WFS17", 
                 "WFS21", "WFS29", "WFS26", "WFS05", "WFS11", "WFS30", "WFS07", "WFS12", "WFS27", "WFS25", 
                 "WFS08", "WFS20", "WFS13", "WFS09", "WFS10", "WFS19", "WFS04", "WFS15", "WFS01")
namevector <- c(Wnamevector, Snamevector)
...
circos.track(factors = full_dat$segment, ylim = c(0,1), track.height = uh(1.5, "mm"),
             panel.fun = function(x, y) {
               if(grepl("WFS", namevector[CELL_META$sector.numeric.index])){
                 circos.text(CELL_META$xcenter, 1.5 + uy(4, "mm"), 
                             namevector[CELL_META$sector.numeric.index], facing = "reverse.clockwise", cex = 0.5)
               } else {
                 circos.text(CELL_META$xcenter, 1.5 + uy(4, "mm"), 
                             namevector[CELL_META$sector.numeric.index], facing = "clockwise", cex = 0.5)
...
#par(mfrow=c(1,1))
#par(mar=c(1,1,1,1))
#tiff("/Users/rishidek/Dropbox/RishiMAC/Genome/PostPhaseScaffoldingFastas/wtdbg2coverage_40_MOLECOL.tiff",height=8,width=20,units="in",res=300,compression="lzw")
...
for (i in 0:39){
  ...
  over40 <- sum(cov_df_subset$V3 > 40)
  under40 <- sum(cov_df_subset$V3 < 41)
  cov_title = paste("WFS", as.character((as.integer(i)+1)), sep = "")
  if(i%%10 == 0){
...
#start by producing schematic of figure
homology_lengthsdf <- read.csv(file = "wtdbg2ChromosomeLenghts.txt", header = FALSE)
homology_lengthsdf <- subset(homology_lengthsdf, homology_lengthsdf$V1 > 50)
homology_lm <- read.csv(file = "LM_stats.txt", header = FALSE, sep = " ")
homology_lengthsdf$Scaffold <- 0:39
homology_lengthsdf$chrom <- 1:40

starts <- as.data.frame(c(1:40))
for (i in 1:40){
  starts$Name[i] <- paste("W", homology_lengthsdf$chrom[i], sep = "")
  starts$startbp <- 0
}

ends <- as.data.frame(c(1:40))
for (i in 1:40){
  ends$Name[i] <- paste("W", homology_lengthsdf$chrom[i], sep = "")
  ends$startbp[i] <- homology_lengthsdf$V1[i]
}

full_start_end <- rbind(starts, ends)
for (i in 1:length(full_start_end$Name)){
  if (nchar(as.character(full_start_end$Name))[i] == 2){
    full_start_end$Name[i] <- as.character(gsub("W", "W0", as.character(full_start_end$Name)[i]))
  }
}
full_start_end <- full_start_end[order(full_start_end$Name),]
full_start_end <- full_start_end[,2:3]

links_dataframe <- read.csv("/Users/rishidek/Dropbox/RishiMAC/Genome/SalmonidSyntenyAnalysis/SyntenyTable.csv")

links_refineddf <- read.csv("/Users/rishidek/Dropbox/RishiMAC/Genome/SalmonidSyntenyAnalysis/Syntenyhomologues.csv")

links_Plottingdf <- links_refineddf

for (i in 1:length(links_Plottingdf$Scaff)){
  scaff_from_subset <- subset(homology_lengthsdf, as.character(homology_lengthsdf$Scaffold) == links_Plottingdf$Scaff[i])
  links_Plottingdf$Scaff_endbp[i] <- scaff_from_subset$V1
}

for (i in 1:length(links_Plottingdf$sutherland.synteny.1.scaffold)){
  scaff_from_subset <- subset(homology_lengthsdf, as.character(homology_lengthsdf$Scaffold) == links_Plottingdf$sutherland.synteny.1.scaffold[i])
  links_Plottingdf$suth_endbp[i] <- scaff_from_subset$V1
}
links_Plottingdf$Scaff <- (as.integer(links_Plottingdf$Scaff) + 1)
links_Plottingdf$sutherland.synteny.1.scaffold <- (as.integer(links_Plottingdf$sutherland.synteny.1.scaffold) + 1)


for (i in 1:length(links_Plottingdf$Scaff)){
  if (nchar(as.character(links_Plottingdf$Scaff))[i] == 1){
    links_Plottingdf$Scaff[i] <- paste("W0", links_Plottingdf$Scaff[i], sep = "")}
  if (nchar(as.character(links_Plottingdf$Scaff))[i] == 2){
    links_Plottingdf$Scaff[i] <- paste("W", links_Plottingdf$Scaff[i], sep = "")}
  if (nchar(as.character(links_Plottingdf$sutherland.synteny.1.scaffold))[i] == 1){
    links_Plottingdf$sutherland.synteny.1.scaffold[i] <- paste("W0", links_Plottingdf$sutherland.synteny.1.scaffold[i], sep = "")}
  if (nchar(as.character(links_Plottingdf$sutherland.synteny.1.scaffold))[i] == 2){
    links_Plottingdf$sutherland.synteny.1.scaffold[i] <- paste("W", links_Plottingdf$sutherland.synteny.1.scaffold[i], sep = "")}
}

library(circlize)
circos.clear()
#name of file/parameters
#tiff("/Users/rishidek/Dropbox/RishiMAC/Genome/PostPhaseScaffoldingFastas/homologoustest.tiff",height=8,width=8,units="in",res=300,compression="lzw")
# Customize plot parameters
par(fig=c(0,1,0,1),mar=c(1,1,1,1))

#create names vector
namesvec <- as.character(1:40)

#then set circos plot parameters including the rotation, gaps between segments and gaps between tracks
circos.par("track.height" = 0.08 , start.degree=87, cell.padding = c(0.005, 0, 0.005, 0), gap.degree = 2, track.margin = c(0.0045, 0.0045))
#initialize
circos.initialize(factors = full_start_end$Name, x = full_start_end$startbp)
...
circos.track(factors = full_start_end$Name, ylim = c(0,1),
             panel.fun = function(x, y) {
               circos.text(CELL_META$xcenter, 0.9 + uy(2, "mm"), 
                           namesvec[CELL_META$sector.numeric.index], cex = 0.8)
             })

#loop through linkage data frame and extract info to make links
for (i in 1:length(links_Plottingdf$Scaff)){
  a <- links_Plottingdf$Scaff[i]
  b <- as.integer(links_Plottingdf$Scaff_endbp[i])
  c <- links_Plottingdf$sutherland.synteny.1.scaffold[i]
  d <- as.integer(links_Plottingdf$suth_endbp[i])
  circos.link(as.character(a), c(0, b), as.character(c), c(0, d), col = colvec[i])
}

#####################
# now plot symap output data
#figure7 MOLECOLRESOURCES 
#now load links we got from symap
symaplinks <- read.csv("../../../PhilineRishi/GenomePaper/Symap_Output/whitefish_blocks.tsv", sep = "\t")
symaplinkssimilarity <- read.csv("../../../PhilineRishi/GenomePaper/Symap_Output/block_percent_similarities.txt", sep = "\t", header = FALSE)

#add link similarity
symaplinks_full <- cbind(symaplinks, symaplinkssimilarity)

for (i in 1:length(symaplinks_full$Chr1)){
  if (nchar(as.character(symaplinks_full$Chr1))[i] == 1){
    symaplinks_full$Chr1[i] <- paste("W0", symaplinks_full$Chr1[i], sep = "")}
  if (nchar(as.character(symaplinks_full$Chr1))[i] == 2){
    symaplinks_full$Chr1[i] <- paste("W", symaplinks_full$Chr1[i], sep = "")}
  if (nchar(as.character(symaplinks_full$Chr2))[i] == 1){
    symaplinks_full$Chr2[i] <- paste("W0", symaplinks_full$Chr2[i], sep = "")}
  if (nchar(as.character(symaplinks_full$Chr2))[i] == 2){
    symaplinks_full$Chr2[i] <- paste("W", symaplinks_full$Chr2[i], sep = "")}
}

library(circlize)
circos.clear()
#name of file/parameters
tiff("/Users/rishidek/Dropbox/PhilineRishi/GenomePaper/Symap_Output_withsimilarity_MOLECOL.tiff",height=8,width=8,units="in",res=300,compression="lzw")
# Customize plot parameters
par(fig=c(0,1,0,1),mar=c(1,1,1,1))
...
circos.track(factors = full_start_end$Name, ylim = c(0,0.1), track.height = uh(1, "mm"),
             panel.fun = function(x, y) {
             })

circos.track(factors = full_start_end$Name, ylim = c(0,1),
             panel.fun = function(x, y) {
               if(nchar(namesvec[CELL_META$sector.numeric.index]) < 2){
                 circos.text(CELL_META$xcenter, 1.5 + uy(1.5, "mm"), 
                             paste("WFS0", namesvec[CELL_META$sector.numeric.index], sep =""), cex = 0.4)
               } else {
                 circos.text(CELL_META$xcenter, 1.5 + uy(1.5, "mm"), 
                             paste("WFS", namesvec[CELL_META$sector.numeric.index], sep =""), cex = 0.4)
               }
})


#loop through linkage data frame and extract info to make links
# for (i in 1:nrow(symaplinks_full)){
#   a <- symaplinks_full$Chr1[i]
#   b <- as.integer(symaplinks_full$Start1[i])
#   c <- as.integer(symaplinks_full$End1[i])
#   d <- symaplinks_full$Chr2[i]
#   e <- as.integer(symaplinks_full$Start2[i])
#   f <- as.integer(symaplinks_full$End2[i])
#   circos.link(as.character(a), c(b, c), as.character(d), c(e, f), col = colvec[i])
# }
# 
# 
# for (i in 1:nrow(symaplinks_full)){
...
#   e <- as.integer(symaplinks_full$Start2[i])
#   f <- as.integer(symaplinks_full$End2[i])
#   circos.link(as.character(a), c(b, c), as.character(d), c(e, f), col = (add_transparency("grey", transparency = 0.7)))
# }

#symaplinks_full$similarity <- as.integer(symaplinks_full$V2)

#round the values of similary from many dp to whole %s
symaplinks_full$similarity <- round((symaplinks_full$V2), digits=0)
...
#   b <- as.integer(symaplinks_full$Start1[i])
#   c <- as.integer(symaplinks_full$End1[i])
#   colourname <- as.character(symaplinks_full$colour[i])
#   circos.rect(b, 0, c, 1, as.character(a), track.index = 1, col = colourname, border = NA)
# }

#now try colouring by value: red orange yellow green blue purple
symaplinks_full$cat_col <- "empty"
for (i in 1:(nrow(symaplinks_full))){

  if ((symaplinks_full$similarity)[i] == 86){
    symaplinks_full$cat_col[i] <- "black"
  }
  if ((symaplinks_full$similarity)[i] == 87){
    symaplinks_full$cat_col[i] <- "blue"
  }
  if ((symaplinks_full$similarity)[i] == 88){
    symaplinks_full$cat_col[i] <- "green"
  }
  if ((symaplinks_full$similarity)[i] == 89){
    symaplinks_full$cat_col[i] <- "yellow"
  }
  if ((symaplinks_full$similarity)[i] == 90){
    symaplinks_full$cat_col[i] <- "orange"
  }
  if ((symaplinks_full$similarity)[i] > 90){
    symaplinks_full$cat_col[i] <- "red"
  }
}
...
for (i in 1:(nrow(symaplinks_full))){
  ...
  b <- as.integer(symaplinks_full$Start1[i])
  c <- as.integer(symaplinks_full$End1[i])
  circos.rect(b, 0, c, 1, as.character(a), track.index = 2, col = symaplinks_full$cat_col[i], border = NA)
}

#read in bed file of collapsed scaffolds:
collapsed_wtdbg2 <- read.csv("wtdbg2_collapsed_figure7.csv")
for (i in 1:(nrow(collapsed_wtdbg2))){
  if (nchar(collapsed_wtdbg2$scaffold)[i] < 2){
    collapsed_wtdbg2$scaffold[i] <- paste("W0", collapsed_wtdbg2$scaffold[i], sep = "")
  } else {
    collapsed_wtdbg2$scaffold[i] <- paste("W", collapsed_wtdbg2$scaffold[i], sep = "")
  }
}

#add collapsed regions in grey on ourter track
for (i in 1:(nrow(collapsed_wtdbg2))){
  ab <- as.integer(collapsed_wtdbg2$start[i])
  ba <- as.integer(collapsed_wtdbg2$end[i])
  circos.rect(ab, -0.01, ba, 0.11, as.character(collapsed_wtdbg2$scaffold[i]), track.index = 1, col = "grey50", border = NA)
}

#and add links between homeologous regions
for (i in 1:nrow(symaplinks_full)){
  a <- symaplinks_full$Chr1[i]
  b <- as.integer(symaplinks_full$Start1[i])
  c <- as.integer(symaplinks_full$End1[i])
  ...
  e <- as.integer(symaplinks_full$Start2[i])
  f <- as.integer(symaplinks_full$End2[i])
  circos.link(as.character(a), c(b, c), as.character(d), c(e, f), col = add_transparency(symaplinks_full$cat_col[i], transparency = 0.7))
}

legend("topright", c(">90%", "90%", "89%", "88%", "87%", "86%"), col = c("red", "orange", "yellow", "green", "blue", "black"), lwd = 1.5, cex = 0.55) #title = "MapQ of 4744 links")
dev.off()

#want to calc length that is confidently > 90% similar 
redorange <- subset(symaplinks_full, symaplinks_full$cat_col == "red" | symaplinks_full$cat_col == "orange")
redorange$proportion <- (redorange$End1-redorange$Start1)
sum(redorange$proportion)
(sum(redorange$proportion)/2200000000)*100

#want to calc length that is confidently > 90% similar 
red <- subset(symaplinks_full, symaplinks_full$cat_col == "red")
red$proportion <- (red$End1-red$Start1)
sum(red$proportion)
(sum(red$proportion)/2200000000)*100

#and just the total length that is in the symap plot
duplicated <- symaplinks_full
duplicated$proportion <- (duplicated$End1-duplicated$Start1)
sum(duplicated$proportion)
(sum(duplicated$proportion)/2200000000)*100
