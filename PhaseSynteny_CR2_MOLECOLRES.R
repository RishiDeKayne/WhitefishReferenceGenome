#PhaseSynteny_CR2_MOLECOLRES.R
#upadted analysis of Canu genome assembly for De-Kayne, Zoller and Feulner 2020
library(circlize)
#load in chromosome lengths and linkage map lengths
setwd("Dropbox/RishiMAC/Genome/PostPhaseScaffoldingFastas/")
lengthsdf <- read.csv(file = "CanuR2ChromosomeLenghts.txt", header = FALSE)
...
samfile <- read.csv(file = "SA_linkagemap_CanuR2PhaseMapped_Filtered.csv")
...
chrom_order_df
missing <- c("scaffold22", "scaffold39")
...
#tiff("CanuR2Hi_C.vs.LinkageMap.tiff",height=8,width=8,units="in",res=300,compression="lzw")
...
legend("topright", c("30-39 - n=145", "40-49 - n=202", "50-59 - n=216", "60 - n=3931"), col = c("red", "orchid1", "lightblue", "royalblue1"), lwd = 1.5, cex = 0.55, title = "MapQ of 4494 links")
...
for (i in 1:2){
  circos.clear()
  #name of file/parameters
  filename <- paste("CanuHi_C.vs.LinkageMap_LG", as.character(i), ".tiff", sep = "")
  ...
  legendtitle <- paste(as.character(gsub("c", "LG", LGNAME)), "MapQ of 4494 links", sep = " ")
  legend("topright", c("30-39 - n=145", "40-49 - n=202", "50-59 - n=216", "60 - n=3931"), col = c("red", "orchid1", "lightblue", "royalblue1"), lwd = 1.5, cex = 0.55, title = as.character(legendtitle))
  #dev.off()
}
...
######################################################################################################
######################################################################################################
#           MOLECOLRESOURCES UPDATE
######################################################################################################
#---------------------------------
#plot syntey with collapsed duplicates higlighted

collapsed <- read.csv("collapsedduplicates_CANU.csv", header = TRUE)
...
circos.clear()
#name of file/parameters
tiff("/Users/rishidek/Dropbox/RishiMAC/Genome/PostPhaseScaffoldingFastas/CANU_R2_HiC_vs_LM_MOLECOLRES_NEW.tiff",height=8,width=8,units="in",res=300,compression="lzw")
# Customize plot parameters
par(fig=c(0,1,0,1),mar=c(1,1,1,1))
#create gaps vector
gapsnorm <- c(1)
...
# Snamevector <- c("Sca39", "Sca22", "Sca12", "Sca7", "Sca11", "Sca31", "Sca8", "Sca2", "Sca38",
#                  "Sca26", "Sca5", "Sca6", "Sca24", "Sca34", "Sca37", "Sca23", "Sca36", "Sca33", 
#                  "Sca28", "Sca27", "Sca17", "Sca16", "Sca29", "Sca20", "Sca3", "Sca15", "Sca32", 
#                  "Sca35", "Sca14", "Sca30", "Sca25", "Sca9", "Sca4", "Sca21", "Sca13", "Sca10", "Sca18", 
#                  "Sca0", "Sca19", "Sca1")

Snamevector <- c("CS40", "CS23", "CS13", "CS08", "CS12", "CS32", "CS09", "CS03", "CS39",
                 "CS27", "CS05", "CS07", "CS25", "CS35", "CS38", "CS24", "CS37", "CS34", 
                 "CS29", "CS28", "CS18", "CS17", "CS30", "CS21", "CS04", "CS16", "CS33", 
                 "CS36", "CS15", "CS31", "CS26", "CS10", "CS05", "CS22", "CS14", "CS11", "CS19", 
                 "CS01", "CS20", "CS02")
...
circos.track(factors = full_dat$segment, ylim = c(0,1), track.height = uh(1.5, "mm"),
             panel.fun = function(x, y) {
               if(grepl("CS", namevector[CELL_META$sector.numeric.index])){
...
for (i in 1:40){
  ...
  if (lm_stats$coefficients[2] < 0){
    ...
    rec[rec<0]<-0
    new_LG_subset$rec <- rec
    plot(new_LG_subset$VN.0.7.17.r1188, new_LG_subset$rec,type="l", lty = 2, col="blue", ylab="recombination rate [cM/Mb]", xlab = "bp", main = title, xaxt="none")
...
#load coverage data
cov_df <- read.csv("CanuR2_Phase30kb_windows_scaffolds.csv", header = FALSE)
...
for (i in 0:39){
  ...
  over40 <- sum(cov_df_subset$V3 > 40)
  under40 <- sum(cov_df_subset$V3 < 41)
  cov_title = paste((scaffname), "total windows = ", as.character(nrow(cov_df_subset)),sep = "")
...
hist(cov_df_noex$V3, main = paste("Canu - Coverage < 60 ", " total = ", as.character(title_rows)))

cov_df_noex30 <- subset(cov_df, cov_df$V3 < 30)
title_rows30 <- nrow(cov_df_noex30)
hist(cov_df_noex30$V3, main = paste("Canu - Coverage < 30 ", " total = ", as.character(title_rows30)))
...
#par(mfrow=c(1,1))
#par(mar=c(1,1,1,1))
tiff("/Users/rishidek/Dropbox/RishiMAC/Genome/PostPhaseScaffoldingFastas/CANU_R2_coverage_40_MOLECOL.tiff",height=8,width=20,units="in",res=300,compression="lzw")
...
for (i in 0:39){
  ...
  over40 <- sum(cov_df_subset$V3 > 40)
  under40 <- sum(cov_df_subset$V3 < 41)
  cov_title = paste("CS", as.character((as.integer(i)+1)), sep = "")
  if(i%%10 == 0){
    plot(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), main = cov_title, col=cov_df_subset$Colour, xlab = "bp", ylab = "coverage", xaxt="none")
    #abline(lm(cov_df_subset$V3 ~ cov_df_subset$V2), col = "green")
    #now adding grey rectangles for partial collapsed duplicates
    if(scaffname == "scaffold0_"){
      rect(c(0), c(-2), c(17900000), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    if(scaffname == "scaffold4_"){
      rect(c(58000000), c(-2), c(max(cov_df_subset$V2)), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    if(scaffname == "scaffold11_"){
      rect(c(15000000), c(-2), c(65000000), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    if(scaffname == "scaffold34_"){
      rect(c(34000000), c(-2), c(max(cov_df_subset$V2)), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    #and adding rectangles for full scaffold collapsed duplicates
    if(scaffname == "scaffold7_"){
      rect(c(0), c(-2), c(max(cov_df_subset$V2)), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    if(scaffname == "scaffold12_"){
      rect(c(0), c(-2), c(max(cov_df_subset$V2)), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    if(scaffname == "scaffold22_"){
      rect(c(0), c(-2), c(max(cov_df_subset$V2)), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    if(scaffname == "scaffold39_"){
      rect(c(0), c(-2), c(max(cov_df_subset$V2)), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
  } else {
    ...
    if(scaffname == "scaffold34_"){
      rect(c(33000000), c(-2), c(max(cov_df_subset$V2)), 42, col = "lightgrey")
      points(cov_df_subset$V3 ~ cov_df_subset$V2, ylim = c(0,40), col=cov_df_subset$Colour)
    }
    #and adding rectangles for full scaffold collapsed duplicates
    if(scaffname == "scaffold7_"){
